...
import de.geotech.systems.geometry.Rectangle2D;
...
public class Projection implements Cloneable {
  ...
  private double totalFalseNorthing = 0;
	 ...
	 */
	public Coordinate transform( Coordinate src, Coordinate dst ) {
		double x = src.x*DTR;
	 ...
	 */
	public Coordinate transformRadians( Coordinate src, Coordinate dst ) {
		double x = src.x;
		if ( projectionLongitude != 0 )
			x = MapMath.normalizeLongitude( x-projectionLongitude );
		project(x, src.y, dst);
	 ...
	 */
	public Coordinate project(double x, double y, Coordinate dst) {
		dst.x = x;
		dst.y = y;
		return dst;
	}
	 ...
	 */
	public Coordinate inverseTransform(Coordinate src, Coordinate dst) {
	 ...
	 */
	public Coordinate inverseTransformRadians(Coordinate src, Coordinate dst) {
		double x = (src.x - totalFalseEasting) / totalScale;
		double y = (src.y - totalFalseNorthing) / totalScale;
		projectInverse(x, y, dst);
		if (dst.x < -Math.PI)
	 ...
	 */
	public Coordinate projectInverse(double x, double y, Coordinate dst) {
		dst.x = x;
		dst.y = y;
		return dst;
	}
